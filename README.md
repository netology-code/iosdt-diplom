...
Все данные по погоде должны кэшироваться в базе данных (можно использовать любой фреймворк, который будет вам удобен - CoreData или Realm и различного рода обертки).
...
Прежде всего, после первого запуска приложения необходимо спросить у пользователя о разрешении на использование геолокации, так как это необходимо, с точки зрения политики конфиденциальности - для этого используется экран, который называется онбординг: 

<img src="/Design/1.jpg" align="middle" height="433" width="200" >

**Основной экран**

После того как пользователь дал или не дал согласие на использование местоположения, он проваливается на следующий экран, который будем именовать основным экраном.

<img src="/Design/3.jpg" align="middle" height="433" width="200" >

Это и есть основной экран приложения, на котором будет отображаться погода, влажность, осадки, длина светового для время, дата и скорость ветра. Дальше речь пойдет об элементах основного экрана
...
На основном экране сверху справа есть кнопка о выборе местоположения. Это кнопка добавления нового города. При нажатии на эту кнопку должен появиться UIAlertController с возможностью ввода города (дизайна так же нет, дейсте стандартный UIAlertController). 
...
При запуске приложения до того, как будет получен ответ о погоде, вам необходимо показывать закэшированные значения погоды.
...
## Виджет *
Дополнительно в дизайне отрисован виджет, можете добавить его в своем проекте, если вам рассказывали как, или вам просто интересно.
...
6. Добавляйте код в проект логически обособленными частями через пул реквесты, это облегчит и ускорит проверку. Код для следующего пул реквеста можно начинать писать, не дожидаясь проверки предыдущего.
